...
| 19 | Faz 9: Cloudflare Turnstile fiyatlandırması ve domain yapılandırması | Web araması ile doğrulandı (2026-09-17) — Turnstile Standard planı tamamen ücretsiz, istek bazlı bir limiti yok (özellik bazlı kısıtlar var: 20 widget'a kadar, genişletilmiş analitik yok). Widget Cloudflare Dashboard'dan `teknoterazi.vercel.app` domain'i için oluşturuldu; yerelde `localhost` test edilirken "Error 110200" (domain uyuşmazlığı) alındı çünkü widget'a `localhost` domain'i eklenmemişti — bu MVP için önemli değil, üretim domain'i doğru çalışıyor. | Yerelde de test edilmek istenirse widget'a `localhost` domain'i eklenmeli |
