...
| 9 | MVP sonrası (backlog) | ✅ Tamamlandı |
...
| 30 | Deneysel dal: frontend'in React/Next.js'e taşınması | Kural #5'in ("ayrı bir frontend framework kullanma, yalnızca Django şablonları + saf HTML/CSS/JS") **main'de hâlâ geçerli** olduğu, henüz değiştirilmediği not edilir. Kullanıcı, `experiment/react-frontend-redesign` dalında bu kısıtı kaldırıp frontend'i tamamen React/Next.js'e taşımayı denemek istedi (2026-09-18) — kapsamlı bir mimari değişiklik olduğu için önce main'e değil, ayrı bir dala alındı. Bu deneme için `.claude/skills/` altına 3 Claude skill'i kuruldu: `frontend-design` (Anthropic, framework-bağımsız görsel tasarım), `web-design-guidelines` (Vercel, 100+ erişilebilirlik/UX kuralına göre kod incelemesi), `ui-ux-pro-max` (NextLevelBuilder, stil/palet/font kütüphanesiyle tam tasarım sistemi üretimi) — bunları birlikte kullanmak için `.claude/skills/frontend-redesign/SKILL.md` yazıldı. React'e özel Vercel skill'leri (React Best Practices, Composition Patterns, React Native) bilinçli olarak kurulmadı (henüz React kodu yok). | Deneme başarılı olup main'e alınırsa: kural #5 ve CLAUDE.md'nin "Stack and hard constraints" bölümü güncellenmeli, Vercel'in React'e özel skill'leri de o zaman kurulmalı |
| 31 | Deneysel dal: `frontend/` (Next.js) ↔ Django API — mimari ve iki ortam gotcha'sı | Django tarafında ayrı bir DRF katmanı **eklenmedi**; `polls/api.py` + `polls/api_urls.py` (`/api/...`) mevcut `views.py`'deki sorgu/servis fonksiyonlarını (`get_poll_with_stats`, `get_trending_polls`, `cast_vote`, `get_voter`/`attach_voter_cookie` vb.) birebir yeniden kullanan düz `JsonResponse` view'ları — DRF eklemek bu iki salt-okunur liste/detay + bir oylama endpoint'i için gereksiz bir bağımlılık olurdu, iş kuralı tamamen `services.py`'de kalıp iki frontend arasında asla sapamaz. `frontend/next.config.ts`'de `/api/:path*` → Django'ya (yerelde `BACKEND_URL`, varsayılan `http://127.0.0.1:8000`) **rewrite** ile proxy'leniyor — bilinçli seçim: tarayıcı hep aynı origin'e (Next sunucusu) istek attığı için CORS veya `SameSite` çerez sorunu hiç çıkmıyor (anonim oy çerezi `tt_voter` ve Django'nun `csrftoken`'ı sorunsuz akıyor); `destination`'a `:path*`'ten sonra elle `/` eklendi çünkü Next'in path segment birleştirmesi trailing slash'i düşürüyor ve Django'nun `APPEND_SLASH` yönlendirmesi sonsuz döngüye giriyordu (`skipTrailingSlashRedirect: true` de gerekti, yoksa Next kendi slash yönlendirmesini rewrite'tan ÖNCE çalıştırıp aynı sorunu yaratıyor). **İki ortam gotcha'sı** (ikisi de sessiz başarısızlık, konsolda hata yok — bir sonraki oturumda tekrar keşfedilmesin): (a) Next.js dev sunucusu `127.0.0.1` üzerinden açılan sekmelerin HMR/RSC isteklerini varsayılan olarak cross-origin diye engelliyor (`⚠ Blocked cross-origin request to Next.js dev resource /_next/hmr` — sadece terminal log'unda görünüyor, tarayıcı konsolunda hiçbir iz yok) ve bu TÜM client-side hydration'ı sessizce kırıyor (tıklamalar hiçbir şey yapmıyor); düzeltme: `next.config.ts`'e `allowedDevOrigins: ["127.0.0.1", "localhost"]` eklendi VE geliştirme sırasında tarayıcıdan her zaman `localhost:3000` kullan (Django backend'e curl/test için `127.0.0.1:8000` kullanmaya devam edilebilir, orada sorun yok — bu port 8000'de Docker'ın da dinlediği bir çakışmadan kaçınmak içindi, bkz. altındaki not). (b) Django'nun CSRF middleware'i `Origin` header'ını (tarayıcı fetch/XHR POST'larında her zaman gönderilir, curl'de gönderilmez — bu yüzden curl testleri yanıltıcı şekilde hep geçti) `request.get_host()`'a göre değil `CSRF_TRUSTED_ORIGINS`'e göre doğruluyor; `http://localhost:3000` bu listede yoktu (`.env`'deki değer eski Django-şablon dünyasından kalma `http://localhost:8000` idi) → tüm oylar sessizce "CSRF doğrulaması başarısız" HTML'i ile 403 dönüyordu (`castVote`'un `res.json()`'ı bu HTML'i parse edemeyince "Unexpected token '<'" hatası fırlatıyordu, gerçek sebep değildi). Düzeltme: `.env` ve `.env.example`'daki `CSRF_TRUSTED_ORIGINS`'e `http://localhost:3000` eklendi. | main'e alınırsa: bu iki .env/next.config ayarı normal geliştirme dokümantasyonuna taşınmalı; production'da Vercel'de bu ikinci frontend'in nasıl deploy edileceği (ayrı proje mi, `vercel:vercel-services` mi) hâlâ karara bağlanmadı |
| 32 | Deneysel dal: React tarafında auth (giriş/kayıt/profil/anketlerim) | Aynı desen (#31) devam ettirildi: `accounts/api.py` + `accounts/api_urls.py` (`/api/auth/...`) mevcut `EmailLoginForm`/`SignUpForm`/`UsernameChangeForm`/`verify_turnstile_token`'ı birebir kullanıyor, Django'nun kendi `login()`/`logout()` fonksiyonlarını çağırıyor (bu yüzden `polls/signals.py`'deki anonim-oy-taşıma sinyali otomatik tetikleniyor, ayrıca kod yazmaya gerek kalmadı). Kayıt/giriş/çıkış/profil/kullanıcı adı değiştirme/anketlerim (listele+kapat/aç+sil) React tarafında tarayıcıdan gerçek bir kullanıcıyla uçtan uca doğrulandı. Turnstile widget'ı (`frontend/src/components/turnstile-widget.tsx`) gerçek Cloudflare script'ini yükleyip render ediyor; site key `frontend/.env.local`'da `NEXT_PUBLIC_TURNSTILE_SITE_KEY` olarak (public bir değer, gizli değil) tutuluyor. Yerelde beklendiği gibi Karar #19'daki AYNI sınırlama tekrar çıktı (hata 110200, `localhost` widget'ın izinli domain listesinde değil) — bu React tarafına özel bir hata değil, düzeltmesi de aynı (Cloudflare Dashboard'da widget'a domain eklemek), bu yüzden dokunulmadı. Anket silme onayı için ayrı bir sayfa yerine tarayıcının yerleşik `window.confirm()`'ü kullanıldı (basitlik) — otomatik tarayıcı testlerinde bu diyalog tetiklenmemeli (oturumu kilitler), bu yüzden `Sil` butonuna tıklamadan endpoint doğrudan curl ile test edildi. | Gerçek signup'ı yerelde test etmek istenirse widget'a `localhost` eklenmeli (Karar #19'daki not); `window.confirm()` yerine özel bir onay modalı istenirse ayrı bir madde olarak ele alınabilir |
| 33 | Deneysel dal: React tarafında anket oluşturma (`/anket-olustur`) | Aynı desen (#31/#32): `polls/api.py`'deki `poll_create` view'ı mevcut `PollForm`/`ProductFormSet`/`upload_product_image`'ı Django'nun kendi formset mekanizmasıyla **birebir** çağırıyor — React tarafı sadece Django'nun formset alan adlandırmasını (`products-TOTAL_FORMS`, `products-0-name` vb.) taklit eden bir `multipart/form-data` gönderiyor (`frontend/src/lib/poll-client.ts`), validasyon mantığı hiç kopyalanmadı. 2-5 ürünlü dinamik satır ekleme/çıkarma, tüm alan hataları (poll/ürün/non-form/görsel yükleme) ve gerçek yayınlama tarayıcıdan gerçek bir kullanıcıyla uçtan uca doğrulandı (görsel dosya yükleme UI'da var ama tarayıcı testinde denenmedi — sadece link alanı test edildi; backend'in `upload_product_image` çağrısı curl ile ayrıca doğrulanmıştı, bkz. Faz 9 karar #25). | Görsel dosya yükleme yolu tarayıcıdan da test edilebilir; `poll_edit` (oy gelmeden anket düzenleme, Django tarafında zaten var) henüz React'e taşınmadı |
| 34 | Deneysel dal: React tarafında yorum ve şikayet | Aynı desen (#31/#32/#33): `polls/api.py`'ye eklenen `comment_add`/`comment_delete`/`report_poll` mevcut `CommentForm`/`ReportForm`'u ve tekil-şikayet `IntegrityError` yakalamasını birebir kullanıyor. Yorum formu her `ProductCard`'ın altına gömülü (`CommentSection` bileşeni); giriş yapılmamışsa Django şablonundaki gibi "giriş yap" linkine dönüşüyor. Şikayet, orijinal `report_form.html` gibi ayrı bir sayfada (`/anket/[id]/sikayet`) — yalnızca giriş yapmış VE anket sahibi olmayan kullanıcıya gösteriliyor. Ayrıca hiç bağlanmamış duran "Bu anketi paylaş" butonu da bu geçişte gerçek panoya-kopyalama işlevine kavuşturuldu (`ShareButton`). Yorum ekleme/silme ve şikayet gönderme (tekrar şikayeti reddetme dahil) tarayıcıdan iki farklı gerçek kullanıcıyla (biri anket sahibi, biri değil) uçtan uca doğrulandı. | — |
| 35 | Deneysel dal: React tarafında anket düzenleme (`/anket/[id]/duzenle`) | Aynı desen (#31-#34): `polls/api.py`'ye eklenen `poll_edit` (GET+POST) mevcut `PollForm`/`ProductForm`'u (formset değil — orijinal view'daki gibi her ürün için ayrı, `products-{ürün_pk}-` önekli bağımsız `ModelForm`) ve "oy almışsa düzenleme kapalı"/"aynı ürün adı iki kez olamaz" kontrollerini birebir kullanıyor. Anket oluşturmadan farklı olarak burada ürün ekleme/çıkarma yok (mevcut ürün seti sabit) ve alan adları index değil gerçek ürün ID'si kullanıyor. Poll detay sayfasına sahibi için eskiden hiç var olmayan "Düzenle/Kapat veya Yeniden Aç/Sil" owner-action butonları da bu geçişte eklendi (`PollOwnerActions`) — "Düzenle" yalnızca `pollHasVotes` false ise gösteriliyor. GET (mevcut verinin forma önceden doldurulması) ve POST (güncelleme) tarayıcıdan gerçek bir kullanıcıyla uçtan uca doğrulandı; backend'de ayrıca "oy aldıktan sonra düzenleme 403 döner" kontrolü curl ile doğrulandı. | Görsel dosya yükleme yolu (edit tarafında da var) tarayıcıdan test edilmedi, sadece backend'den (`upload_product_image`, karar #25) ve create akışından (karar #33) dolaylı doğrulandı |
| 36 | Deneysel dal: Vercel deploy stratejisi — ayrı proje + üç sessiz-başarısızlık gotcha'sı | **Mimari karar**: `frontend/` için mevcut Django projesinin (`teknoterazi`) İÇİNE değil, **ayrı bir Vercel projesine** (`teknoterazi-frontend`, Root Directory: yok/otomatik — CLI'dan `frontend/` içinden deploy ediliyor) deploy edildi; `vercel:vercel-services` (tek projede iki servis) seçeneği CLAUDE.md'nin vercel.json/Django routing uyarısıyla aynı risk kategorisinde olduğu için elenmiş. `next.config.ts`'deki `/api/*` rewrite deseni prod'da da aynen çalışıyor (`BACKEND_URL` env değişkeni hedefi belirliyor), yeni bir CORS/vercel.json ayarına gerek yok. Vercel CLI kurulu değildi, `npx vercel login` ile kuruldu; MCP Vercel bağlantısı bu oturumda hiç yetkilendirilemedi (list_teams boş, list_projects 403) — tüm işlemler CLI ile yapıldı. **Üç sessiz-başarısızlık gotcha'sı** (üçü de "Ready"/200 görünüp gerçekte çalışmıyordu — bir sonraki oturumda tekrar keşfedilmesin): (a) `vercel project add` ile sıfırdan oluşturulan bir projenin **Framework Preset**'i otomatik "Next.js" olmuyor (`null`/"Other" kalıyor) — CLI'dan `vercel build` + `vercel deploy` çalıştırılınca `detectedFramework: skipped` ile sessizce `@vercel/static-build`'e düşüyor (yalnızca `public/` statik dosyalarını topluyor, HİÇ sunucu fonksiyonu üretmiyor), deployment "Ready" ve build log'u "başarılı" görünüyor ama her rota 404 dönüyor (`vercel inspect --json`'daki `output: []` bunun işareti). Düzeltme: `vercel project update <ad> --framework nextjs`, ardından **yerel `.vercel/project.json`'ı `vercel pull` ile yeniden çekmek zorunlu** (proje ayarı API'de değişse de yerel build bu değişikliği görmüyordu). (b) Bir monorepo alt dizininde CLI'dan deploy ederken **Root Directory proje ayarı ile deploy'un çalıştırıldığı dizin uyuşmalı**: `frontend/` içinden `vercel deploy` çalıştırıp proje ayarında Root Directory="frontend" olursa "The specified Root Directory frontend does not exist" hatası (temiz, en azından görünür); ama repo kökünden `VERCEL_ORG_ID`/`VERCEL_PROJECT_ID` env değişkenleriyle deploy edip Root Directory="frontend" bırakılırsa (git-tetiklemeli deploy'ların beklediği kombinasyon) SESSİZCE `output: []` üretiyor — (a)'daki framework sorunuyla aynı belirtiler ama farklı sebep. En güvenilir yol: Root Directory'yi boş/otomatik bırak, HER ZAMAN `frontend/` içinden (kendi `.vercel/project.json`'ıyla) deploy et; büyük fark yaratan asıl adım `vercel build` (yerel) + `vercel deploy --prebuilt` akışına geçmekti — bu, "remote build içinde `vercel build` çalıştırma" tuhaflığını (log'da görülüyordu) tamamen atlıyor. (c) İki farklı Vercel projesinin Preview deploy'ları arasında **sunucu-sunucu istek** (frontend'in Server Component'i Django'ya `fetch` çağrısı) varsayılan olarak Vercel'in Preview SSO korumasına çarpıyor — tarayıcı için 302→giriş sayfası, sunucu tarafı `fetch` için ise 200 dönen bir HTML sayfası (Vercel'in login ekranı), bu da `res.json()`'da "Unexpected token '<'" hatasına yol açıyor (CSRF hatasıyla AYNI belirti, farklı sebep — dikkat). Düzeltme: hedef projede (`teknoterazi`) `vercel project protection --protection-bypass` ile bir "Protection Bypass for Automation" secret'ı etkinleştirilip çağıran projenin (`teknoterazi-frontend`) env değişkenine eklendi, `frontend/src/lib/api.ts`'deki `backendFetch` bunu `x-vercel-protection-bypass` header'ı olarak gönderiyor (yalnızca env değişkeni tanımlıysa — prod'da etkisiz). Ayrıca: Preview ortamının `DATABASE_URL`'i baştan beri `postgres.placeholder` gibi bir yer tutucuymuş (gerçek kimlik bilgisi hiç girilmemiş) — gerçek Supabase bağlantı dizesiyle güncellendi (yalnızca Preview, production'a dokunulmadı). | Deneme main'e alınırsa: `teknoterazi-frontend` projesinin "Production Branch" ayarı ve gerçek bir domain/subdomain kararı henüz verilmedi; şu an her ikisi de sadece Vercel'in otomatik `*.vercel.app` preview URL'lerinde çalışıyor |
