...
- **Auth follows the same pattern** (Karar Günlüğü #32): `accounts/api.py` + `accounts/api_urls.py` (`/api/auth/...`) wrap Django's own `login()`/`logout()` and the existing `EmailLoginForm`/`SignUpForm`/`UsernameChangeForm` — this is why the anon-vote-merge signal (`polls/signals.py`) already fires correctly on React login/signup with no extra code. `frontend/src/components/navbar.tsx` is an async Server Component that calls `fetchCurrentUser()` per request; any change to auth state (login/signup/logout) must call `router.refresh()` afterward so the navbar re-renders. The Turnstile site key is public by design — it lives in `frontend/.env.local` as `NEXT_PUBLIC_TURNSTILE_SITE_KEY`, not a secret needing backend proxying.
...
## Vercel deploy strategy (resolved — see Karar Günlüğü #36)

`frontend/` deploys as its own **separate Vercel project** (`teknoterazi-frontend`), not inside the Django project — `vercel:vercel-services` (one project, two services) was rejected as the same risk category as CLAUDE.md's vercel.json/Django-routing warning. The `/api/*` rewrite in `next.config.ts` works unchanged in the cloud (`BACKEND_URL` env var points at whichever Django deployment to use) — no CORS setup needed there either.

Three silent-failure gotchas hit while setting this up, all looking "Ready"/200 while actually broken — check these before re-diagnosing a "works locally, broken on Vercel" report on this project:
1. **A project created via `vercel project add` does not default its Framework Preset to Next.js.** Building it as-is silently falls back to `@vercel/static-build` (`detectedFramework: skipped` in the build log) — it only collects `public/` static files and produces **zero serverless functions**, so the build log looks clean and the deployment shows "Ready", but every single route 404s (`vercel inspect --json` shows `"output": []`). Fix: `vercel project update <name> --framework nextjs`, then **`vercel pull` again locally** — the local `.vercel/project.json` cache doesn't pick up the change on its own.
2. **Root Directory setting must match where you invoke `vercel deploy` from.** Running from within `frontend/` while the project's Root Directory is set to `"frontend"` fails loudly ("does not exist"). Running from the repo root with `VERCEL_ORG_ID`/`VERCEL_PROJECT_ID` env overrides while Root Directory is still `"frontend"` fails *silently* with the same empty-`output` symptom as #1. The reliable combination: leave Root Directory empty/automatic and always deploy from inside `frontend/` using its own `.vercel/project.json` — and prefer the `vercel build` (local) + `vercel deploy --prebuilt` two-step over a bare `vercel deploy`, which sidesteps an observed oddity where the remote build shells out to `vercel build` internally.
3. **Server-to-server calls between two Vercel projects' Preview deployments hit Vercel's own SSO wall by default.** `backendFetch` in `frontend/src/lib/api.ts` calling a Django *Preview* deployment gets Vercel's login page back (200, HTML) instead of JSON — same `Unexpected token '<'` symptom as the CSRF gotcha above, different cause, so don't assume it's CSRF again. Fix (already in place): enable a Protection Bypass secret on the Django project (`vercel project protection --protection-bypass`), put it in the frontend project's `VERCEL_PROTECTION_BYPASS_SECRET` env var, and `backendFetch` sends it as `x-vercel-protection-bypass` whenever that env var is set — it's a no-op in production, where `BACKEND_URL` points at the public production Django deployment instead.

Also: a Preview environment's env vars are a **separate set** from Production's on the same project — `DATABASE_URL` for Preview had been left as a `postgres.placeholder` value (never real Supabase credentials) until this setup work, which surfaced as a `psycopg.OperationalError` deep in a Django 500 page, not anything frontend-shaped.
